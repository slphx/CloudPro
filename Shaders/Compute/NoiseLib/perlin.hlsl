...
float Perlin3D(float3 p, float frequency = 1.0, bool isTiling = false) {
    ...
    if (!isTiling) {
        perlin = lerp(
            lerp(   lerp(dot(hash33(i + float3(0.0, 0.0, 0.0)), f - float3(0.0, 0.0, 0.0)),
                        dot(hash33(i + float3(1.0, 0.0, 0.0)), f - float3(1.0, 0.0, 0.0)), u.x),
                    lerp(dot(hash33(i + float3(0.0, 1.0, 0.0)), f - float3(0.0, 1.0, 0.0)),
                        dot(hash33(i + float3(1.0, 1.0, 0.0)), f - float3(1.0, 1.0, 0.0)), u.x),
                    u.y),
            lerp(   lerp(dot(hash33(i + float3(0.0, 0.0, 1.0)), f - float3(0.0, 0.0, 1.0)),
                    ...
                    lerp(dot(hash33(i + float3(0.0, 1.0, 1.0)), f - float3(0.0, 1.0, 1.0)),
                        dot(hash33(i + float3(1.0, 1.0, 1.0)), f - float3(1.0, 1.0, 1.0)), u.x),
                    u.y),
            u.z
        );
        ...
        perlin = lerp(
            lerp(   lerp(dot(hash33(i + float3(0.0, 0.0, 0.0), frequency), f - float3(0.0, 0.0, 0.0)),
                        dot(hash33(i + float3(1.0, 0.0, 0.0), frequency), f - float3(1.0, 0.0, 0.0)), u.x),
                    lerp(dot(hash33(i + float3(0.0, 1.0, 0.0), frequency), f - float3(0.0, 1.0, 0.0)),
                        dot(hash33(i + float3(1.0, 1.0, 0.0), frequency), f - float3(1.0, 1.0, 0.0)), u.x),
                    u.y),
            lerp(   lerp(dot(hash33(i + float3(0.0, 0.0, 1.0), frequency), f - float3(0.0, 0.0, 1.0)),
                        dot(hash33(i + float3(1.0, 0.0, 1.0), frequency), f - float3(1.0, 0.0, 1.0)), u.x),
                    lerp(dot(hash33(i + float3(0.0, 1.0, 1.0), frequency), f - float3(0.0, 1.0, 1.0)),
                        dot(hash33(i + float3(1.0, 1.0, 1.0), frequency), f - float3(1.0, 1.0, 1.0)), u.x),
                    u.y),
            u.z
        );
    }
